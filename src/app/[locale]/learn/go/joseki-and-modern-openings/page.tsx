...
import {
  ...
  getJosekiAndModernOpeningsOutline,
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: t('labels.metadataTitle', {
      title: t('josekiAndModernOpenings.title'),
    }),
    description: t('josekiAndModernOpenings.description'),
  };
}

export default async function JosekiAndModernOpeningsPage() {
  const t = await getTranslations('go.lessons');
  const common = await getTranslations('common');
  const lessons = getGoLessons(t);
  const lessonIndex = lessons.findIndex(
    ({ id }) => id === 'josekiAndModernOpenings',
  );
    ...
    >
      <Container className="py-10">
        ...
        <div className="grid items-start gap-x-6 gap-y-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]">
          <LessonOutline
            items={getJosekiAndModernOpeningsOutline(t)}
            labels={{
