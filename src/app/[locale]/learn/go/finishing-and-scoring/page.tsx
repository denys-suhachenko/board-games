...
import {
  FinishingAndScoringLesson,
  getFinishingAndScoringOutline,
} from '@/features/games/go/ui/lessons/FinishingAndScoringLesson';

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('go.lessons');
  return {
    title: t('labels.metadataTitle', { title: t('finishingAndScoring.title') }),
    description: t('finishingAndScoring.description'),
  };
}

export default async function FinishingAndScoringPage() {
  const t = await getTranslations('go.lessons');
  const common = await getTranslations('common');
  const lessons = getGoLessons(t);
  const lessonIndex = lessons.findIndex(
    ({ id }) => id === 'finishingAndScoring',
  );
    ...
    >
      <Container className="py-10">
        ...
        <div className="grid items-start gap-x-6 gap-y-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]">
          <LessonOutline
            items={getFinishingAndScoringOutline(t)}
            labels={{
