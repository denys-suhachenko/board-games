...
import {
  LegalMovesAndKoLesson,
  getLegalMovesAndKoOutline,
} from '@/features/games/go/ui/lessons/LegalMovesAndKoLesson';

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('go.lessons');
  return {
    title: t('labels.metadataTitle', { title: t('legalMovesAndKo.title') }),
    description: t('legalMovesAndKo.description'),
  };
}

export default async function LegalMovesAndKoPage() {
  const t = await getTranslations('go.lessons');
  const common = await getTranslations('common');
  const lessons = getGoLessons(t);
  const lessonIndex = lessons.findIndex(({ id }) => id === 'legalMovesAndKo');
  ...
  return (
    <NextIntlClientProvider
      messages={{
        go: {
          lessons: {
            exercises: { board: messages.go.lessons.exercises.board },
    ...
    >
      <Container className="py-10">
        ...
        <div className="grid items-start gap-x-6 gap-y-8 lg:grid-cols-[minmax(0,1fr)_minmax(0,3fr)]">
          <LessonOutline
            items={getLegalMovesAndKoOutline(t)}
            labels={{
